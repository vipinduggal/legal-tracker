#!/bin/bash
cat > ~/legal-tracker/src/manager.html << 'ENDHTML'
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
...
.btn{display:inline-flex;align-items:center;gap:5px;padding:7px 13px;border-radius:var(--r);border:1px solid var(--g2);background:#fff;cursor:pointer;font-size:13px;font-weight:500;color:var(--g7);font-family:var(--f);transition:all .12s;text-decoration:none}
...
<div class="wrap">
  <div class="ph">
    ...
    <div style="display:flex;gap:8px">
      <button class="btn" onclick="researchPending()" id="rp-btn">&#128269; Research pending</button>
  ...
  <div class="card">
    <div class="add-form" id="add-form">
      <div class="form-row">
        <div class="fg"><label>Company name *</label><input id="nn" placeholder="e.g. Apple Inc" oninput="chk()"></div>
        <div class="fg"><label>Industry *</label>
          <select id="ni" onchange="chk()">
        ...
        <div class="fg"><label>HQ location *</label><input id="nl" placeholder="e.g. Cupertino, CA" oninput="chk()"></div>
      </div>
      <div style="display:flex;gap:8px">
    ...
    <div class="card-hdr">
      ...
      <div class="filters">
        <input placeholder="Search..." oninput="render()" id="srch">
        <select onchange="render()" id="sf">
  ...
  tb.innerHTML = f.map(function(a) {
    ...
    return '<tr id="row-' + a.id + '">'
      ...
      + '<td><div class="btn-row">'
        ...
        + '<button class="btn btn-sm" id="' + rid + '" onclick="doResearch(\'' + a.id + '\',\'' + safeName + '\')">' + rLabel + '</button>'
        + '<button class="btn btn-sm btn-d" onclick="showModal(\'' + a.id + '\',\'' + safeName + '\')">Remove</button>'
      + '</div></td>'
      + '</tr>';
  }).join('');
}
...
async function addAccount() {
  ...
  try {
    ...
    rem.textContent = 'Added: ' + name + '. Click Research to start, or it runs automatically at 7 AM tomorrow.';
    rem.style.display = 'block';
    setTimeout(function(){ rem.style.display = 'none'; }, 8000);
    await load();
  } catch(e) { toast(e.message, 'err'); }
  finally { btn.disabled = false; btn.textContent = 'Add account'; validateAdd(); }
}
  ...
  try {
    ...
    var timer = setInterval(async function() {
      try {
        ...
        if (elapsed > 300) { clearInterval(timer); btn.disabled = false; btn.innerHTML = '&#8635; Update'; }
      } catch(e) { clearInterval(timer); btn.disabled = false; btn.innerHTML = '&#8635; Update'; }
    }, 8000);
  } catch(e) {
    toast(e.message, 'err');
    btn.disabled = false;
...
async function researchPending() {
  ...
  toast('Queued ' + pending.length + ' accounts. Check email for updates.', 'ok');
}

function showModal(id, name) {
  delId = id; delName = name;
  document.getElementById('modal-txt').textContent = 'Remove "' + name + '" and all its research data? This cannot be undone.';
