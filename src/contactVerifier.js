...
{"latest_filing":"most recent earnings period or null","cost_initiatives":"cost reduction program with dollar amount or null","earnings_signals":"earnings call comment about costs/legal or null","ma_activity":"M&A deal or null"}` }],
    ...
    if (rolesResp.status === "fulfilled") {
      ...
      // Strategy 1: find array and object separately using regex
      ...
      if (arrMatch) {
        try {
          ...
          if (Array.isArray(parsed)) {
            openRoles = parsed;
            logger.info(`Open roles parsed for ${account.name}: ${openRoles.length} roles`);
          }
        } catch(e) {
          // Try to find last complete item
          const lastBracket = arrMatch[1].lastIndexOf("}");
          if (lastBracket > 0) {
            try {
              openRoles = JSON.parse(arrMatch[1].slice(0, lastBracket + 1) + "]");
              logger.info(`Open roles recovered for ${account.name}: ${openRoles.length} roles`);
            } catch(e2) {
              logger.warn(`Open roles parse failed for ${account.name}: ${e.message}`);
            }
          }
        }
      }
      ...
      if (objMatch) {
        try { financialIntel = JSON.parse(objMatch[1]); } catch(e) {}
      }

      // Strategy 2: if no array found, try parsing whole thing
      if (!openRoles.length) {
        try {
          const parsed = JSON.parse(raw);
          if (Array.isArray(parsed)) openRoles = parsed;
        } catch(e) {}
      }
